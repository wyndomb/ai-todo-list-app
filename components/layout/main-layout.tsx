...
export function MainLayout() {
  ...
  const [activeView, setActiveView] = useState<
    "today" | "upcoming" | "calendar" | "insights"
  >("today");
  const [isProjectsOpen, setIsProjectsOpen] = useState(true);
  const { tasks, categories, filterBy, setFilter, isLoading, fetchCategories } =
    useTodoStore();
  ...
  const todayTasksCount = useMemo(() => {
    ...
    return tasks.filter(
      (task) =>
        !task.completed &&
        (task.dueDate === today || (task.dueDate && task.dueDate < today))
    ).length;
  }, [tasks]);
  ...
  const categoryTaskCounts = useMemo(() => {
    return categories.reduce((acc, category) => {
      acc[category.name] = tasks.filter(
        (task) => !task.completed && task.category === category.name
      ).length;
      return acc;
    }, {} as Record<string, number>);
  }, [tasks, categories]);

  // Desktop navigation items (includes calendar)
  ...
  // Mobile navigation items (replaces calendar with AI assistant)
            ...
            <button
              ...
              onClick={() => {
                if (item.isAiAssistant) {
                  ...
                  handleViewChange(item.id as typeof activeView);
                }
              }}
              className={cn(
